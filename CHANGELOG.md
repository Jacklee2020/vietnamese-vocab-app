...
- **恢复清爽淡蓝主题色**：全面恢复 `#4aa3df` / `#83c4ef` 淡蓝主题色调（顶栏、主按钮、打卡圆环、选中态与渐变阴影）。
- **完整恢复词汇隐藏功能**：恢复 `S.hidden` 自定义词库机制（详情卡「🙈 不再背诵 / 恢复背诵」、词表行快捷「🙈」隐藏按钮、词表底部「已隐藏词汇」管理与一键恢复）。
- **模板与构建体系对齐**：更新 `template.html` 与 v1.7.0 全部特性完全对齐，确保 4,621 词注入后不丢失任何历史功能。
- **自动化测试增强**：`test_app.cjs` 增加主题色与词汇隐藏能力自动化断言（21 项测试全绿通过）。
